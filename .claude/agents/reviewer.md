...
2. **Try to disconfirm the conclusion.** Name the strongest plausible alternative
   explanation or failure scenario supported by the artifacts, then run the cheapest
   counterexample or boundary check that separates it. Do not request private
   chain-of-thought; report only the candidate, test, and observation.
3. **Treat [ASSUMED]/[INFERRED] as the risk list.** For each, decide: acceptable,
   needs verification before merge, or blocker.
4. **Audit the diff against intent.** Every hunk connects to the stated goal in one
...
5. **Security, where touched.** Apply the high-miss set from
...
6. **Guardrails.** Any secrets, destructive operations, or policy violations in the
