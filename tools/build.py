...
Budgets (build FAILS over budget, per marching orders + Amendment 01):
    ...
    adapters/system-prompt/fablized-micro.md  <= 800 words
"""
...
def main():
    ...
    unknown = set(sys.argv[1:]) - {"--force-part2", "--check"}
    if unknown:
        sys.exit(f"BUILD FAILED: unknown option(s): {', '.join(sorted(unknown))}")
    if force_part2 and CHECK_ONLY:
        sys.exit("BUILD FAILED: --force-part2 and --check cannot be combined")
    ...
    if mode == "filled" and not force_part2:
        ...
        print(f"  AGENTS.md: filled Part 2 detected — preserved verbatim; "
              f"Part 1 {action} (--force-part2 overwrites with the skeleton)")
    else:
    ...
    for name in PROTOCOLS:
        ...
        full.append(body)
    full_text = "\n".join(full)
    counts["full"] = check_budget(
        "adapters/system-prompt/fablized-full.md", full_text, FULL_BUDGET)
    ...
    counts["compact"] = check_budget(
        "adapters/system-prompt/fablized-compact.md", compact, COMPACT_BUDGET)
    write(ADAPTERS / "system-prompt" / "fablized-compact.md", compact)

    # Micro: one named action-loop digest, fully inlined — no skills or files to
    # trigger. It is deliberately not the compact profile with words removed ad hoc.
    ...
    profile_manifest = {
        ...
        "profiles": {
            "full": {"file": "fablized-full.md", "words": counts["full"], "budget": FULL_BUDGET},
            "compact": {
                "file": "fablized-compact.md",
                "words": counts["compact"],
    ...
    write(
        ADAPTERS / "system-prompt" / "profiles.json",
    ...
    if DRIFT:
        sys.exit("BUILD CHECK FAILED: generated outputs differ: " + ", ".join(DRIFT))
    if CHECK_ONLY:
        print("build check passed: generated outputs match core/")
    print("done.")
