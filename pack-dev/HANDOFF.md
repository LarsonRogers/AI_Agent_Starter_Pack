...
**Branches:** `main` — now at **v12.18**, carrying both features (v12.17 Light-tier usage reporting + v12.18 Right-sized & Resilient), each independently reviewed (APPROVE, 0 blockers), fast-forward-merged and pushed to `origin/main`. The merged feature branches `feature/tier-use-reporting` + `feature/right-sized-resilient` are now redundant (safe to delete). `eval-testing` — the ★-subset run plan (pushed, separate concern).
